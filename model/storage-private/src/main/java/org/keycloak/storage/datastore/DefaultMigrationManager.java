...
public class DefaultMigrationManager implements MigrationManager {
	...
	public void migrate() {
		...
		ModelVersion databaseVersion = model.getStoredVersion() != null ? new ModelVersion(model.getStoredVersion())
				: null;
		...
		if (databaseVersion == null || databaseVersion.lessThan(latestUpdate)) {
			...
			if (databaseVersion.equals(SNAPSHOT_VERSION)) {
				throw new ModelException("Incorrect state of migration. You are trying to run server version '"
						+ currentVersion + "' against a database which was migrated to snapshot version '"
						+ databaseVersion
						+ "'. Databases that have been migrated to a snapshot version can't be migrated to a released version of Keycloak or to a more recent snapshot version.");
			} else {
				logger.warnf("Possibly incorrect state of migration. You are trying to run server version '"
						+ currentVersion + "' against database, which was already migrated to newer version '" +
						databaseVersion + "'.");
			}
		}
	...
	public static final ModelVersion SNAPSHOT_VERSION = new ModelVersion("25.0.0-SNAPSHOT");
	...
	public void migrate(RealmModel realm, RealmRepresentation rep, boolean skipUserDependent) {
		...
		if (stored == null) {
			...
			if (currentVersion.lessThan(stored)) {
				logger.warnf(
						"Possibly incorrect state of migration during realm import. You are running server version '"
								+ currentVersion
								+ "' when importing JSON file, which was created in the newer version '" +
								stored + "'.");
			}
		}
	...
	public static ModelVersion convertRHSSOVersionToKeycloakVersion(String version) {
		...
		// chceck if the version is in format for CD releases, e.g.: "keycloakVersion":
		// "6"
